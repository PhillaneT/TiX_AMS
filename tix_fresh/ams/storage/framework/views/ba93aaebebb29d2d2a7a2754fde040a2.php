...
<head>
    ...
    <title><?php echo $__env->yieldContent('title', 'AjanaNova AMS'); ?></title>
...
<aside class="w-64 bg-navy-900 flex flex-col fixed inset-y-0 left-0 z-40">
    ...
    <?php $ctx = auth()->check() ? \App\Models\ActiveContext::with(['qualification','cohort'])->where('user_id', auth()->id())->first() : null; ?>
    <div class="mx-3 mt-4 mb-2 p-3 rounded-lg <?php echo e($ctx?->cohort_id ? 'bg-[#e3b64d]' : 'bg-navy-700 border border-dashed border-navy-500'); ?>">
        <?php if($ctx?->qualification_id): ?>
            <p class="text-xs uppercase tracking-wide text-[#011c38] font-bold">Currently Assessing</p>
            <p class="text-white text-sm font-semibold mt-0.5 leading-tight"><?php echo e(Str::limit($ctx->qualification->name ?? '—', 28)); ?></p>
            <?php if($ctx?->cohort_id): ?>
                <p class="text-brand-200 text-xs mt-0.5"><?php echo e($ctx->cohort->name ?? ''); ?></p>
            <?php else: ?>
        ...
        <?php else: ?>
            ...
            <a href="<?php echo e(route('dashboard')); ?>" class="text-brand-400 text-xs hover:underline">Set your context →</a>
        <?php endif; ?>
    </div>
    ...
    <nav class="flex-1 px-3 py-3 space-y-1 overflow-y-auto">
        <a href="<?php echo e(route('dashboard')); ?>"
           class="sidebar-link <?php echo e(request()->routeIs('dashboard') ? 'active' : 'text-[#e3b64d]'); ?>">
        ...
        <a href="<?php echo e(route('qualifications.index')); ?>"
           class="sidebar-link <?php echo e(request()->routeIs('qualifications.*') ? 'active' : 'text-[#e3b64d]'); ?>">
        ...
        <?php if($ctx?->qualification_id): ?>
        <a href="<?php echo e(route('qualifications.assignments.index', $ctx->qualification_id)); ?>"
           class="sidebar-link <?php echo e(request()->routeIs('qualifications.assignments.*') ? 'active' : 'text-[#e3b64d]'); ?>">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
            Assignments
        </a>
        <?php else: ?>
        <a href="<?php echo e(route('qualifications.index')); ?>" class="sidebar-link text-[#e3b64d]">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
            Assignments
        ...
        <a href="<?php echo e(route('integrations.index')); ?>"
           class="sidebar-link <?php echo e(request()->routeIs('integrations.*') ? 'active' : 'text-[#e3b64d]'); ?>">
    ...
    <div class="border-t border-navy-700 p-4">
        <div class="flex items-center gap-3">
            <div class="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 bg-[#e3b64d]">
                <span class="text-white text-xs font-bold"><?php echo e(strtoupper(substr(auth()->user()->name ?? 'A', 0, 1))); ?></span>
            </div>
            <div class="flex-1 min-w-0">
                <p class="text-white text-sm font-medium truncate"><?php echo e(auth()->user()->name ?? 'Assessor'); ?></p>
                <p class="text-slate-400 text-xs truncate"><?php echo e(auth()->user()->email ?? ''); ?></p>
            </div>
        </div>
...
<div class="ml-64 flex-1 flex flex-col min-h-screen">
    ...
    <header class="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
        <div>
            <h1 class="text-lg font-semibold text-gray-900"><?php echo $__env->yieldContent('heading', 'Dashboard'); ?></h1>
            <?php if (! empty(trim($__env->yieldContent('breadcrumbs')))): ?>
                <nav class="flex items-center flex-wrap gap-1 text-sm text-gray-500 mt-0.5">
                    <?php echo $__env->yieldContent('breadcrumbs'); ?>
                </nav>
            <?php endif; ?>
        </div>
        <div class="flex items-center gap-3">
            <?php if(config('app.mock_mode', true)): ?>
...
<?php /**PATH /home/runner/workspace/ams/resources/views/layouts/app.blade.php ENDPATH**/ ?>