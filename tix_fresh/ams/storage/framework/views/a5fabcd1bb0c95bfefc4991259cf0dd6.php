<?php $__env->startSection('title', 'Qualification Modules — ' . $qualification->name); ?>
<?php $__env->startSection('heading', 'Qualification Modules'); ?>
...
<script>
const AMS_ASSIGNMENTS = <?php echo json_encode($assignments->map(fn($a) => [
    'id'    => $a->id, 'label' => $a->name . ' (' . ucfirst($a->type) . ')', ])->values()) ?>;

const AMS_MAPPING = <?php echo json_encode($mapping, 15, 512) ?>;
</script>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-1 space-y-6">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            ...
            <form method="POST" action="<?php echo e(route('qualifications.modules.fetch-saqa', $qualification)); ?>">
                <?php echo csrf_field(); ?>
                <label class="block text-xs font-medium text-gray-600 mb-1">SAQA Qualification ID</label>
                <input type="text" name="saqa_id"
                       value="<?php echo e(old('saqa_id', $qualification->saqa_id)); ?>"
            ...
            <?php if($qualification->saqa_fetched_at): ?>
                <p class="mt-2 text-xs text-gray-400">Last fetched: <?php echo e($qualification->saqa_fetched_at->diffForHumans()); ?></p>
            <?php endif; ?>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            ...
            <form method="POST" action="<?php echo e(route('qualifications.modules.add', $qualification)); ?>">
                <?php echo csrf_field(); ?>
                <div class="space-y-2">
                    <div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm text-sm">
            ...
            ?>
            <?php $__currentLoopData = $byType; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $type => $mods): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="flex justify-between py-1 border-b border-gray-50 last:border-0">
                <span class="font-medium text-gray-600"><?php echo e($type); ?> (<?php echo e($mods->count()); ?>)</span>
                <span class="text-gray-500"><?php echo e($mods->sum('credits')); ?> credits</span>
            </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            <div class="flex justify-between pt-2 font-semibold text-gray-800">
                <span>Total (<?php echo e($modules->count()); ?>)</span>
                <span><?php echo e($totalCredits); ?> credits</span>
            </div>
        </div>
        <?php endif; ?>
    ...
    <div class="lg:col-span-2">
        ...
        <?php else: ?>
            <form method="POST" action="<?php echo e(route('qualifications.modules.save-mapping', $qualification)); ?>" id="mapping-form">
                <?php echo csrf_field(); ?>
                <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                    <div class="flex items-center justify-between px-5 py-3 bg-gray-50 border-b border-gray-200">
                        <h2 class="font-semibold text-gray-800 text-sm">
                            <?php echo e($modules->count()); ?> Module(s) — Module to Activity Mapping
                        </h2>
                        <button type="submit"
                    ...
                    <?php if($assignments->isEmpty()): ?>
                        <div class="px-5 py-4 text-xs text-amber-700 bg-amber-50 border-b border-amber-100">
                            No assignments yet. <a href="<?php echo e(route('qualifications.assignments.create', $qualification)); ?>" class="underline font-semibold">Create assignments first</a>, then map them here.
                        </div>
                    <?php endif; ?>
                    ...
                    <div class="divide-y divide-gray-100" id="module-rows">
                        ...
                        <div class="grid grid-cols-12 px-5 py-3 items-start gap-y-1"
                             data-mod-id="<?php echo e($mod->id); ?>"
                             data-mapped='<?php echo json_encode($mapped, 15, 512) ?>'>
                            
                            <div class="col-span-1 pt-0.5">
                                <code class="text-xs text-red-500 break-all leading-tight"><?php echo e($mod->module_code); ?></code>
                            </div>
                            
                            <div class="col-span-1 pt-0.5">
                                <span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-bold <?php echo e($badgeCls); ?>">
                            ...
                            <div class="col-span-4 pt-0.5 pr-2">
                                <div class="text-sm font-medium text-gray-800 leading-tight"><?php echo e($mod->title); ?></div>
                            </div>
                            
                            <div class="col-span-1 pt-0.5 text-center text-sm text-gray-600">
                                <?php echo e($mod->nqf_level ?? '—'); ?>

                            </div>
                            
                            <div class="col-span-1 pt-0.5 text-center text-sm text-gray-600">
                                <?php echo e($mod->credits ?: '—'); ?>

                            </div>
                            
                            <div class="col-span-4">
                                <div class="space-y-1.5 map-rows" id="rows-<?php echo e($mod->id); ?>">
                                    
                                </div>
                                <button type="button"
                                        onclick="addRow(<?php echo e($mod->id); ?>)"
                                        class="mt-1.5 text-xs text-blue-600 hover:text-blue-800 font-semibold flex items-center gap-1 add-btn"
                                        id="add-<?php echo e($mod->id); ?>">
                                    + Add activity
                                </button>
                            </div>
                        </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/runner/workspace/ams/resources/views/qualifications/modules.blade.php ENDPATH**/ ?>